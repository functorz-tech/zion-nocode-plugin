...
> **Always invoke the CLI exactly as written above** — via `npx -y zion-mcp@2.7.7` — never bare `zion-mcp`, even though `--help` prints its own name that way. The `@2.7.7` version pin runs this plugin's exact published build, so a globally-installed `zion-mcp` (or `zion`) on `PATH` can't shadow it.
