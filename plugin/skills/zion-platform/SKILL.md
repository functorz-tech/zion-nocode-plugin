...
```bash
npx -y zion-mcp@2.6.1 projects search --projectName "My App"
npx -y zion-mcp@2.6.1 project set-current --projectExId <exId>
npx -y zion-mcp@2.6.1 schema load
```
...
> **Always invoke the CLI exactly as written above** — via `npx -y zion-mcp@2.6.1` — never bare `zion-mcp`, even though `--help` prints its own name that way. The `@2.6.1` version pin runs this plugin's exact published build, so a globally-installed `zion-mcp` (or `zion`) on `PATH` can't shadow it.
