...
### Scope Note
ACTIVATE_PAYMENT turns the module on one type at a time, and the actionflow plugin edits the generated flows' bodies. What you cannot do is the user's own editor work (Action → Payment): the gateway credentials, the brand assets and the plan upgrade. Design the order table first (via the database plugin), then activate, then wire the secure order-creation flow and add the `alreadyProcessed` idempotency guard in the payment flow.
